...
public class DigitalObjectUtil {
	...
	public static JsonObject rowToJsonObject(Row row, JsonNode columnMappingNode, boolean includeTypeAndId) {
	...
	public static void addAttributeFromRow(JsonNode columnMappingNode, String key, Row row, JsonElement jElement) {
		...
		if (colIndexNode == null || colIndexNode.isNull()) {
			...
			if (colIndexNode.isInt()) {
				...
				if ((generateScopedIdNode != null && generateScopedIdNode.asBoolean(false))
						&& (cellValue == null || (cellValue instanceof String && ((String) cellValue).isEmpty()))) {
					...
					// To-Do: How to make sure that the ID does not exist in the local scope of the
					// object?
	...
	public static void addAttributeToJsonElement(Object value, JsonElement jElement, String key) {
		if (jElement.isJsonObject()) {
			...
			if (value instanceof Integer) {
				...
				jobject.addProperty(key, (long) value);
				...
				jobject.addProperty(key, (double) value);
			} else if (value == null) {
				// do not add the attribute unless it is for the ID column which might be null
				// before creation
				if (key.equals("id")) {
