...
ConfigurationDialog.launch = function () {
  const frame = $(
    DOM.loadHTML("dissco-uploader", "scripts/dialogs/configuration-dialog.html")
  );
  ...
  elmts.saveButton.click(function () {
    let numberOfProcessingThreads = parseInt(
      elmts.numberOfProcessingThreads.val()
    );
      ...
      },
      {
        onDone: function (data) {
          if (data.code === "ok") {
            ...
            alert(
              "Error - schema could not be saved. Message: " + data.message
            );
          }
        },
...
ConfigurationDialog.setConnectionDefaults = function (elmts) {
  elmts.inputDOServer.val("https://sandbox.dissco.tech/api/v1/specimens");
